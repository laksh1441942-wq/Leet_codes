...
class Solution {
    TreeNode* build(vector<int>& inorder, int inStart, int inEnd, vector<int>& postorder, int postStart, int postEnd, map<int, int>& mp){
        ...
        root->left = build(inorder, inStart, inRoot-1, postorder, postStart, postStart+LeftNum-1, mp );

        root->right = build(inorder, inRoot+1, inEnd, postorder, postStart+LeftNum, postEnd-1, mp );
